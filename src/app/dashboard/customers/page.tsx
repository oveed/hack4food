...
export default function Page(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    async function fetchStudents() {
      try {
        const response = await fetch('http://192.168.1.224:5000/api/students/');
  ...
  const paginatedCustomers = applyPagination(students, page, rowsPerPage);

  return (
    <Stack spacing={3}>
      <Stack direction="row" spacing={3}>
        <Stack spacing={1} sx={{ flex: '1 1 auto' }}>
          <Typography variant="h4">Etudiants</Typography>
